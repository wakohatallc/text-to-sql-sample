...
サンプル注文データは2016-2018年台である。MVPの代表ユースケースでは「今週」を2018年内の週として扱い、基準日を `2018-08-29` に固定する。したがって「今週一週間」は `2018-08-27 00:00:00` 以上 `2018-09-03 00:00:00` 未満の期間を指す。本番化時はこの固定基準日を廃止し、実日付基準へ切り替える。
...
| AI SDK | Vercel AI SDK Core | `streamText`、tool calling、usage取得を利用する |
      ...
      },
      {
        ...
        "data": {
          ...
          "rows": [
            ...
            {
              "seller_id": "d13e50eaa47b4cbe9eb81465865d8cfc",
              "total_sales": "726.08",
              "order_count": 4
            }
          ]
...
6. ユーザー入力とschema contextをAI SDKのOpenAI providerへ渡し、SQL生成toolを実行する。
7. 生成SQLをASTで検証する。
...
- MVPの代表ユースケースでは、相対日付の「今週」は `2018-08-29` を基準日として解釈する。
- `2018-08-29` 基準の「今週一週間」は `2018-08-27 00:00:00` 以上 `2018-09-03 00:00:00` 未満である。
...
- 「今週一週間の売り上げランキング上位10位を出して」でSQL生成、検証、実行、結果保存まで通ること。
...
MVPでは「今週」を `2018-08-29` 基準で解釈するため、`2018-08-27 00:00:00` 以上 `2018-09-03 00:00:00` 未満の売上ランキングが返ることを合格条件とする。
...
- AI SDK `streamText` とtool callingを実装する。
- `generateSql`, `validateSql`, `executeSql` の責務を分ける。
